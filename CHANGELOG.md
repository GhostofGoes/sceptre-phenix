...
- **Web UI**: Tables start unpaginated in a fresh browser. The Paginate toggle is remembered in the browser's local storage, shared by every table and every user of that browser, and kept through logout and reloads; it is no longer stored per user.
